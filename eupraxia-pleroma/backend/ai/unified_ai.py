...
class UnifiedAI:
    ...
    def __init__(self):
        self.setup_logging()
        # Try to initialize heavy generators; if imports or GPU are unavailable, fall back to lightweight placeholders
        use_lightweight = False
        heavy_ok = True
        try:
            # Try heavy imports (may fail on CPU-only / missing dependencies)
            from superhuman_ai_coder import SuperhumanAICoder  # type: ignore
            from superhuman_image_generator import SuperhumanImageGenerator  # type: ignore
            from superhuman_video_generator import SuperhumanVideoGenerator  # type: ignore
            from superhuman_3d_model_generator import Superhuman3DModelGenerator  # type: ignore
            from superhuman_music_generator import SuperhumanMusicGenerator  # type: ignore
            from superhuman_writing_generator import SuperhumanWritingGenerator  # type: ignore
            from superhuman_science_generator import SuperhumanScienceGenerator  # type: ignore
            from superhuman_engineering_generator import SuperhumanEngineeringGenerator  # type: ignore
            from superhuman_business_generator import SuperhumanBusinessGenerator  # type: ignore
            from superhuman_art_generator import SuperhumanArtGenerator  # type: ignore
            from superhuman_health_generator import SuperhumanHealthGenerator  # type: ignore
            from superhuman_education_generator import SuperhumanEducationGenerator  # type: ignore
            from superhuman_meta_evolution import SuperhumanMetaEvolution  # type: ignore
            # Advanced generators
            from generators.superhuman_website_generator import SuperhumanWebsiteGenerator  # type: ignore
            from generators.superhuman_game_generator import SuperhumanGameGenerator  # type: ignore
            from generators.superhuman_assistant_generator import SuperhumanAssistantGenerator  # type: ignore
        except Exception as e:
            self.logger.info(f"Heavy generator imports failed or unavailable: {e}")
            heavy_ok = False

        # If GPU not available, prefer lightweight to avoid downloads and OOM
        if not torch.cuda.is_available():
            self.logger.info("CUDA not available: forcing lightweight generator mode.")
            heavy_ok = False

        if heavy_ok:
            try:
                # Initialize heavy generators
                ...
                self.logger.info("Initialized heavy generators successfully")
            except Exception as e:
                self.logger.info(f"Failed initializing heavy generators: {e}")
                heavy_ok = False

        if not heavy_ok:
            # Fallback to lightweight, CPU-friendly implementations
            self.logger.info("Initializing lightweight generators (CPU-friendly placeholders)")
            from .lightweight_generators import (
                LightweightAICoder,
            ...
            self.superhuman_ai_coder = LightweightAICoder()
            self.superhuman_image_generator = LightweightImageGenerator()
            self.superhuman_video_generator = LightweightVideoGenerator()
            self.superhuman_3d_model_generator = Lightweight3DGenerator()
            self.superhuman_music_generator = LightweightMusicGenerator()
            # Minimal writing/science/engineering/business/art/health/education map to AI coder if not present
            self.superhuman_writing_generator = self.superhuman_ai_coder
            self.superhuman_science_generator = self.superhuman_ai_coder
            self.superhuman_engineering_generator = self.superhuman_ai_coder
            self.superhuman_business_generator = self.superhuman_ai_coder
            self.superhuman_art_generator = self.superhuman_ai_coder
            self.superhuman_health_generator = self.superhuman_ai_coder
            self.superhuman_education_generator = self.superhuman_ai_coder

            self.superhuman_website_generator = LightweightWebsiteGenerator()
            self.superhuman_game_generator = LightweightGameGenerator()
            self.superhuman_assistant_generator = LightweightAssistantGenerator()
            self.superhuman_meta_evolution = LightweightMetaEvolution()
